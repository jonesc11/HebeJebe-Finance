	...
	public static String processRequest(String r) throws JSONException {
		...
		if(actionType == "GetTransactions") {
			...
			for(int i = 0; i < allTransactions.size(); i++) {
				...
				if(allTransactions.get(i) instanceof Income && (allTransactions.get(i).getCategory() == category || category == null) && (types.contains("Income") || types == null)) { 
		...
		// Issues: Currently ignores NextTokens.
			...
			if(action.getString("ResourceIdentifier") != null) {
				...
				for(int i = 0; i < accountsList.size(); i++) {
					...
					o.put("ResourceIdentifier", user.getAccounts().indexOf(accountsList.get(i)));
